...
static TextLayer *s_temp_forecast_text_layer;
static Layer *s_temp_layer;
static GRect s_temp_now_frame;
static GRect s_temp_forecast_frame;
...
static void temp_create(Layer *parent_layer) {
  ...
  s_temp_now_text_layer = text_layer_create(GRect(0, 0, 0, 0));
  ...
  s_temp_forecast_text_layer = text_layer_create(GRect(0, 0, 0, 0));
  text_layer_set_background_color(s_temp_forecast_text_layer, GColorClear);
  text_layer_set_text_color(s_temp_forecast_text_layer, GColorWhite);
  text_layer_set_font(s_temp_forecast_text_layer, s_s_font);
  text_layer_set_text_alignment(s_temp_forecast_text_layer, GTextAlignmentCenter);
  layer_add_child(s_temp_layer, text_layer_get_layer(s_temp_forecast_text_layer));  

}

static void temp_update_frame() {
...
void complications_destroy() {
  ...
  layer_destroy(text_layer_get_layer(s_temp_forecast_text_layer));
...
void temp_update() {
  if((g_time.hours >= 7 && g_time.pm) || (g_time.hours < 7 && g_time.pm)) {
    ...
    layer_set_frame(text_layer_get_layer(s_temp_now_text_layer), s_temp_now_frame);
    text_layer_set_text(s_temp_now_text_layer, g_temp_now_buffer);

    s_temp_forecast_frame = (GRect) {
      .origin = GPoint(0, COMPLICATION_L_FONT_H+1),
      .size = (GSize) {
        .w = COMPLICATION_W,
        .h = COMPLICATION_S_FONT_H
      },
    };
    layer_set_frame(text_layer_get_layer(s_temp_forecast_text_layer), s_temp_forecast_frame);
    text_layer_set_text(s_temp_forecast_text_layer, g_temp_low_buffer);
    ...
    layer_set_frame(text_layer_get_layer(s_temp_forecast_text_layer), s_temp_forecast_frame);
    text_layer_set_text(s_temp_forecast_text_layer, g_temp_high_buffer);
